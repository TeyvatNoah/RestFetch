...
{
	public static unsafe int UnescapePercentEncodedUTF8Sequence(char* input, int length, ref ValueStringBuilder dest, bool iriParsing)
	{
		...
		Debug.Assert(UriHelper.DecodeHexChars(input[1], input[2]) != UriFromBCL.c_DummyChar);
	...
	DecodeRune:
		...
		if (Rune.DecodeFromUtf8(new ReadOnlySpan<byte>(&temp, bytesLeftInBuffer), out Rune rune, out bytesConsumed) == OperationStatus.Done)
		{
			...
			if (!iriParsing || IriHelper.CheckIriUnicodeRange((uint)rune.Value))
			{
				if (charsToCopy != 0)
				{
